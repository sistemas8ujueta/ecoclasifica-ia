...
import type { UsuarioLocalAdmin } from "../services/auth";
...
export function Usuarios() {
  ...
  const [usuarios, setUsuarios] = useState<UsuarioLocalAdmin[]>([]);
  ...
  useEffect(() => {
    ...
    async function cargarUsuarios() {
      try {
        const usuariosLocales = auth.listarUsuariosLocales();

        const usuariosConEscaneos = await Promise.all(
          usuariosLocales.map(async (usuario) => {
            try {
              const historial = await storage.obtenerHistorial(usuario.id);

              return {
                ...usuario,
                totalClasificaciones: historial.length,
              };
            } catch (error) {
              console.error(
                `No se pudo obtener el historial de ${usuario.nombre}:`,
                error
              );
        ...
        if (vigente) {
          setUsuarios(usuariosConEscaneos);
          setCargando(false);
        }
      } catch (error) {
    ...
    <div className="pantalla-columna">
      <div className="usuarios-header">
        ...
        <div className="db-chip">
          ...
          Usuarios almacenados en este dispositivo
        </div>
      </div>
      ...
      <div className="usuarios-lista">
        ...
        {!cargando && filtrados.length === 0 && (
          <EstadoVacio
            ...
            descripcion={
              usuarios.length === 0
                ? "Las cuentas que se registren aparecerán aquí."
                ...
                <div className="info">
                  ...
                  <div className="meta">
                    ...
                    <span className="fecha">
                      Desde {formatearFecha(u.fechaRegistro)}
                    </span>
                  </div>
                </div>
